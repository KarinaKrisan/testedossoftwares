...
export function updatePersonalView(uidOrName) {
    renderWeekendDuty(); // FDS Sempre visível
...
export function renderWeekendDuty() {
    ...
    weekends.forEach(wk => {
        ...
        if (satWorkers.length > 0 || sunWorkers.length > 0) {
            ...
            if (satWorkers.length) html += `<div class="flex items-start gap-1.5"><span class="text-[8px] text-[#40E0D0] font-bold uppercase w-5 mt-0.5">Sáb</span><div class="flex flex-wrap gap-1 flex-1">${satWorkers.map(n => `<span class="px-1.5 py-px rounded bg-white/5 border border-white/5 text-[8px] text-gray-300 flex items-center gap-1 whitespace-nowrap"><div class="w-1 h-1 rounded-full bg-[#40E0D0]"></div>${n.split(' ')[0]}</span>`).join('')}</div></div>`;
            if (sunWorkers.length) html += `<div class="flex items-start gap-1.5"><span class="text-[8px] text-[#4169E1] font-bold uppercase w-5 mt-0.5">Dom</span><div class="flex flex-wrap gap-1 flex-1">${sunWorkers.map(n => `<span class="px-1.5 py-px rounded bg-white/5 border border-white/5 text-[8px] text-gray-300 flex items-center gap-1 whitespace-nowrap"><div class="w-1 h-1 rounded-full bg-[#4169E1]"></div>${n.split(' ')[0]}</span>`).join('')}</div></div>`;
            html += `</div></div>`;
        }
    });
