            ...
            container.innerHTML = `
                <div class="premium-glass p-4 rounded-xl border-l-4 border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.1)]">
                    ...
                    <button id="btnRevokeInvite" class="w-full bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all">
                        <i class="fas fa-ban mr-1"></i> Revogar Link
                    </button>
                </div>
            `;
            ...
            container.innerHTML = `
                <div class="premium-glass p-4 rounded-xl border border-white/5 border-dashed hover:border-white/10 transition-all group">
                    ...
                    <p class="text-[9px] text-gray-500 mb-3 leading-relaxed">
                        Gere um link para adicionar novos membros à equipe.
                    </p>
...
function selectRole(e, key) {
    document.querySelectorAll('.role-option').forEach(el => {
        ...
        el.querySelector('.role-check-circle').classList.remove('border-purple-500');
        el.querySelector('.role-check-dot').classList.remove('bg-purple-500');
    });
    const selectedDiv = e.currentTarget;
    selectedDiv.classList.add('border-purple-500', 'bg-purple-500/10');
    selectedDiv.querySelector('.role-check-circle').classList.add('border-purple-500');
    selectedDiv.querySelector('.role-check-dot').classList.add('bg-purple-500');
...
async function confirmPromotion() {
    ...
    if (!targetUid) return showNotification("Selecione um colaborador.", "error");
    if (!roleKey) return showNotification("Selecione um cargo.", "error");
    ...
    askConfirmation(`Promover ${targetUser.name} para ${config.label}?`, async () => {
        try {
            await updateDoc(getCompanyDoc("users", targetUid), {
                ...
                updatedAt: serverTimestamp(), _claimsRefresh: Date.now(), promotedBy: state.currentUser.email
            });
...
async function internalApplyLogFilter() {
    ...
    onSnapshot(q, (snap) => {
        allLoadedLogs = snap.docs.map(d => {
            const data = d.data();
            let dateStr = data.timestamp?.toDate ? data.timestamp.toDate().toLocaleString('pt-BR') : "---";
            return { id: d.id, date: dateStr, admin: data.adminEmail || 'Sistema', action: data.action || 'Ação', target: data.target || '-' };
        });
...
function renderAuditLogs() {
    ...
    container.innerHTML = `<div class="premium-glass p-4 rounded-xl h-[calc(100vh-140px)] flex flex-col mt-2"><div class="flex justify-between mb-4"><h3 class="text-xs font-bold text-white uppercase"><i class="fas fa-fingerprint text-blue-400"></i> Auditoria</h3></div><div class="overflow-y-auto custom-scrollbar flex-1 space-y-2">` + 
...
function renderMigrationTool() {
    ...
    btn.innerHTML = '<i class="fas fa-database mr-2"></i> Migrar Dados Antigos';
    btn.onclick = () => { askConfirmation("Migrar dados?", runLegacyMigration); };
...
async function runLegacyMigration() {
    try {
        ...
        for (const d of oldCollabsSnap.docs) {
            const data = d.data();
            await setDoc(getCompanyDoc("users", d.id), { name: data.nome||data.name, email: data.email, role: 'collaborator', level: 10, cargo: data.cargo||'Colaborador', setorID: data.setorID||'NOC', horario: data.horario||'08:00', active: true, migratedAt: serverTimestamp() }, { merge: true });
        }
        showNotification("Migração concluída", "success");
...
export function populateEmployeeSelect() {
    ...
    if(s && state.scheduleData) { 
        s.innerHTML = '<option value="">Selecionar...</option>' + Object.keys(state.scheduleData).sort().filter(n=>state.scheduleData[n].level < 100).map(n=>`<option value="${n}">${n}</option>`).join('');
    }
}
