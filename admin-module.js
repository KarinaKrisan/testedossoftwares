...
export async function switchAdminView(view) {
    ...
    if (view === 'Daily' || view === 'daily') {
        ...
        // CORREÇÃO: Força o mês atual ao entrar na Dashboard para garantir dados de HOJE
        const now = new Date();
        if (state.selectedMonthObj.year !== now.getFullYear() || state.selectedMonthObj.month !== now.getMonth()) {
            ...
            // Recarrega os dados do mês correto
...
// --- DASHBOARD (Lógica de Férias e Status Corrigida) ---
export function renderDailyDashboard() {
    ...
    if (state.scheduleData) {
        Object.values(state.scheduleData).forEach(emp => {
            ...
            // Lógica de Férias e Afastamentos (Prioridade Alta)
            if (s === 'FE') {
                g = 'Ferias'; // Se hoje está marcado como FE, vai para o card Férias
            } else if (s === 'A') {
                g = 'Afastado';
            } else if (s === 'LM') {
                g = 'Licenca';
            }
            // Lógica de Trabalho e Folga
