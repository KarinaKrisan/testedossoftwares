...
// --- EXPORTAÇÕES GLOBAIS (Necessárias para os onclicks do HTML) ---
...
window.askConfirmation = askConfirmation; // Helper global
...
export function initAdminUI() {
    ...
    renderEditToolbar(); 
    initApprovalsTab(); 
    renderInviteWidget(); // Nova função de convites
...
// --- DASHBOARD DIÁRIO (A FUNÇÃO QUE FALTAVA) ---
export function renderDailyDashboard() {
    const todayIndex = new Date().getDate() - 1; 
    const groups = { Ativo: [], Encerrado: [], Folga: [], Ferias: [], Afastado: [], Licenca: [] };
    
    // Elementos do DOM onde os cards serão renderizados
    const dashboardContainer = document.getElementById('dailyGrid') || document.getElementById('screenDaily');
    
    // Se o layout antigo de colunas existir, usa ele. Se não, cria estrutura.
    // Vamos assumir a estrutura do HTML antigo para compatibilidade:
    // listAtivo, countAtivo, etc.
    ...
    // Renderiza cada grupo se os elementos existirem no HTML
    Object.keys(groups).forEach(k => renderGroup(k, groups[k]));
}

// --- GESTÃO DE CONVITES (WIDGET NOVO) ---
            ...
            if (!snap.empty) {
                ...
                div.innerHTML = `
                    <h3 class="text-[10px] font-bold text-white uppercase mb-2 flex justify-between">
                        <span><i class="fas fa-link text-emerald-400 mr-1"></i> Link Ativo</span>
                    </h3>
                ...
                document.getElementById('btnRevokeInvite').onclick = () => {
                    askConfirmation("Revogar convite?", async () => {
                        await updateDoc(getCompanyDoc("convites", inviteCode), { active: false });
                        showNotification("Revogado");
                    });
                };
            } else {
                div.innerHTML = `
                    <h3 class="text-[10px] font-bold text-white uppercase mb-1">Novo Colaborador</h3>
                    <button id="btnGenerateInvite" class="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-2 rounded text-[9px] font-bold uppercase shadow-lg transition-all active:scale-95">
                        Gerar Link
                    </button>
                `;
                container.innerHTML = '';
...
export function populateEmployeeSelect() {
    ...
    if(s) { 
        ...
        Object.keys(state.scheduleData || {}).sort().forEach(n => {
            ...
            // Não mostra CEO/Diretor na lista de edição manual comum para evitar acidentes
...
function renderEditToolbar() {
    ...
    toolbar.innerHTML = tools.map(t => `
        <button onclick="window.setEditTool('${t.id}')" class="px-2.5 py-1.5 rounded-lg bg-white/5 border ${t.border} flex items-center gap-1.5 hover:bg-white/10 transition-all">
            <i class="fas ${t.icon} ${t.color} text-[9px]"></i>
            <span class="text-[8px] font-bold text-white uppercase">${t.label}</span>
        </button>
    `).join('');
}

function setEditTool(id) { 
    activeTool = (id === 'null' || id === null) ? null : id; 
    showNotification(activeTool ? `Ferramenta: ${activeTool}` : "Modo Automático");
}
...
export function handleAdminCellClick(name, i) {
    ...
    // Se tiver ferramenta selecionada usa ela, senão rotaciona
...
function openPromoteModal() {
    ...
    Object.values(state.scheduleData).sort((a,b)=>a.name.localeCompare(b.name)).forEach(user => {
        if (user.uid !== state.currentUser.uid) {
            userSelect.innerHTML += `<option value="${user.uid}">${user.name} (${user.cargo || '-'})</option>`;
        }
    });
    ...
    askConfirmation(`Promover ${targetUser.name} para ${config.label}?`, async () => {
        try {
            await updateDoc(getCompanyDoc("users", targetUid), {
                cargo: config.label, role: config.role, level: config.level, promotedBy: state.currentUser.email
            });
...
function renderAuditLogs() {
    ...
    onSnapshot(q, (snap) => {
        ...
        <div class="premium-glass p-4 rounded-xl border border-white/5 h-[calc(100vh-140px)] flex flex-col mt-2">
            <h3 class="text-xs font-bold text-white mb-4">Auditoria</h3>
            <div class="overflow-y-auto custom-scrollbar flex-1 space-y-2">
                ${snap.docs.map(d => {
                    const l = d.data();
                    const time = l.timestamp?.toDate().toLocaleString() || '--';
                    return `<div class="bg-white/5 p-2 rounded border border-white/5"><div class="flex justify-between"><span class="text-[9px] text-blue-300 font-bold">${l.action}</span><span class="text-[8px] text-gray-500">${time}</span></div><p class="text-[9px] text-gray-300">${l.target}</p><p class="text-[8px] text-gray-500">Por: ${l.adminEmail}</p></div>`;
                }).join('')}
            </div>
        </div>`;
    });
...
function initApprovalsTab() {
    ...
    onSnapshot(q, (snap) => {
        ...
        if(snap.empty) { 
            targetDiv.innerHTML = '<p class="text-center text-gray-500 text-[10px]">Nada pendente.</p>'; 
        } else {
...
function askConfirmation(msg, onConfirm) {
    ...
    const newBtn = btnConfirm.cloneNode(true); // Remove listeners antigos
