// main.js - Versão Reativa (Hierarquia Dinâmica)
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        try {
            ...
            // --- CARREGAMENTO DE PERFIL EM TEMPO REAL (NOVO) ---
            const userDocRef = getCompanyDoc("users", user.uid);
            let isFirstLoad = true;

            onSnapshot(userDocRef, (docSnap) => {
                if (!docSnap.exists()) {
                     console.error("Perfil não encontrado"); return; 
                }
                ...
                // Feedback Visual de Promoção
                if (!isFirstLoad && myLevel > oldLevel) {
                    showNotification(`Permissões atualizadas: ${newData.cargo}`, "success");
                    // Toca um som ou animação se quiser
                }

                // Ajusta interface baseado no novo nível
                updateDynamicMenu();

                if (state.isDualRole) {
                    setInterfaceMode('admin'); // Ou mantém o atual, mas habilita o botão
                } else {
                    setInterfaceMode('collab');
                }
                
                // Recarrega dados pois agora pode ver mais coisas
                loadData(); 
                isFirstLoad = false;
...
function buildUserObj(uid, profile, schedule) {
    let safeSchedule = Array.isArray(schedule) ? schedule.map(d => (d === undefined || d === null) ? "" : d) : [];
    const userObj = {
        uid: uid, name: profile.name || profile.nome || "Usuário", 
        role: profile.role || 'collaborator', level: profile.level || 10,
        cargo: profile.cargo || '-', setorID: profile.setorID || 'NOC',
        email: profile.email || "", horario: profile.horario || "08:00 às 17:00",
