...
function setupEvents() {
    ...
    const datePicker = document.getElementById('datePickerTrigger'); if (datePicker) datePicker.onclick = () => { document.getElementById('miniCalendarDropdown').classList.toggle('hidden'); renderMiniCal(); };
    const prev = document.getElementById('miniPrev'); if (prev) prev.onclick = () => { mini.m--; if (mini.m < 0) { mini.m = 11; mini.y-- }; renderMiniCal(); };
    const next = document.getElementById('miniNext'); if (next) next.onclick = () => { mini.m++; if (mini.m > 11) { mini.m = 0; mini.y++ }; renderMiniCal(); };
}
